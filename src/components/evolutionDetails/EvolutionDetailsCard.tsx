...
const EvolutionDetailsCard = ({ details }: Props) => {
  ...
  return (
    <div className="evolution-details">
      ...
      {details.trigger.name === "use-item" && <div className="use-item">Use {item}</div>}
      {details.trigger.name === "level-up" && <div className="level-up">Level up</div>}
      {details.trigger.name === "trade" && <div className="trade">Trade</div>}
      {details.trigger.name === "other" && <div className="trade">?</div>}
      {details.trigger.name === "three-critical-hits" && <div className="three-critical-hits">Land three critical hits in a battle</div>}
      {details.trigger.name === "agile-style-move" && (
        <>
          <div className="three-critical-hits">Use {knownMove}</div>
          <div>20 times in Agile Style</div>
        </>
      )}
      {details.held_item && <div className="holding">holding {heldItem}</div>}
      {details.known_move && details.trigger.name === "level-up" && <div className="knowing-move">knowing {knownMove}</div>}
